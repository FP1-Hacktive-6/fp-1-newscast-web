...
const ProgrammingItem = ({title, description, url, urlToImage}) => {
return (
<div
    className="relative m-3 flex w-full max-w-[48rem] flex-row rounded-xl bg-slate-400 bg-clip-border text-gray-700 shadow-md">
    <div
        className="relative m-0 w-2/5 shrink-0 overflow-hidden rounded-xl rounded-r-none bg-white bg-clip-border text-gray-700">
        <img src={urlToImage} alt={urlToImage} className="h-full w-full object-cover" />
    </div>
    <div className="p-6">
        <h6
            className="mb-4 block font-sans text-base font-semibold uppercase leading-relaxed tracking-normal text-white antialiased">
            Programming
        </h6>
        <h4
